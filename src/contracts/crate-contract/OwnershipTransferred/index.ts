import type {
  eventLog,
  handlerContext,
  SupplyUpdate,
  Crate_OwnershipTransferred_eventArgs,
  OwnershipTransferred,
} from "generated"
import { CHAIN_ID_TO_CHAIN_NAME } from "../../../constants/common"

import dotenv from "dotenv"
import { rabbitMqService } from "../../../connection"
import {
  EXCHANGE_COLLECTION_CREATOR_FLOW,
  RABBITMQ_COLLECTION_CREATOR_FLOW_KEY,
} from "../../../constants/queues"
import { ECrateContractMessageType } from "../constants"

dotenv.config()

export async function ownershipTransferredHandler({
  context,
  event,
}: {
  context: handlerContext
  event: eventLog<Crate_OwnershipTransferred_eventArgs>
}): Promise<void> {
  try {
    await rabbitMqService.connect()
    const { block, chainId, params, srcAddress, transaction } = event
    const { oldOwner, newOwner } = params

    const ownershipTransferred: OwnershipTransferred = {
      id: `${chainId}_${transaction.hash}`,
      chain: CHAIN_ID_TO_CHAIN_NAME[chainId as keyof typeof CHAIN_ID_TO_CHAIN_NAME],
      block_number: block.number,
      block_timestamp: block.timestamp,
      transaction_hash: transaction.hash,
      contract_address: srcAddress,
      created_at: new Date(),
      updated_at: new Date(),

      old_owner: oldOwner,
      new_owner: newOwner,
    }

    const message = {
      type: ECrateContractMessageType.SupplyUpdate,
      data: ownershipTransferred,
    }

    rabbitMqService.sendMessageToExchange({
      exchange: EXCHANGE_COLLECTION_CREATOR_FLOW,
      routingKey: RABBITMQ_COLLECTION_CREATOR_FLOW_KEY,
      message,
    })

    context.OwnershipTransferred.set(ownershipTransferred)

    context.log.info(`OwnershipTransferred event processed: ${JSON.stringify(message)}`)
  } catch (error) {
    context.log.error(`Error processing OwnershipTransferred event: ${error}`)
    throw error
  }
}
